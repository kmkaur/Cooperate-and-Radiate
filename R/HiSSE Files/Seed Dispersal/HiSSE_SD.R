...
taxa <- read.csv("Species_SD_pruned_data.csv", row.names=1)
...
pruned2<-nnls.tree(cophenetic(tree),tree,rooted=TRUE)

#convert data to hisse format
taxa2 <- as.matrix(taxa)
sd<-cbind(rownames(taxa2), taxa2[,1])
...
null_two_hisse_SD <- hisse(pruned2, sd, f=samp, hidden.states=TRUE, turnover.anc=c(1,1,2,2),
...
bisse_none_hidden_SD <- hisse(pruned2, sd, hidden.states=FALSE, f=samp, turnover.anc=c(1,2,0,0),
...
bisse_null_SD <- hisse(pruned2, sd, f=samp, hidden.states=FALSE, turnover.anc=c(1,1,0,0),
...
hisse_1hidden_state_SD <- hisse(pruned2, sd, f=samp, hidden.states=TRUE, turnover.anc=c(1,2,0,3),
...
full_hisse_SD <- hisse(pruned2, sd, f=samp, hidden.states = TRUE, turnover.anc=c(1,2,3,4),
                         eps.anc=c(1,2,3,4), trans.rate = trans.rates.hisse.4, root.p = c(0.5, 0, 0.5, 0), output.type = "raw")

####6) null-four hisse####
null_four_hisse_SD <- hisse.null4(pruned2, sd, f=samp, turnover.anc=rep(c(1,2,3,4),2),
